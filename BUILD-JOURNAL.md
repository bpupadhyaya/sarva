...
## `redacted_thinking` blocks were silently dropped by the Anthropic adapter, breaking multi-turn tool use once the model's own safety classifier flagged its reasoning

Round 60. Ruled out `session.py`/`longterm.py`/`vector.py` (exhaustively hardened already -- every angle probed, from malformed-shape session files to dangling tool-call messages from a truncated turn, was either already correctly handled or provably unreachable) and `audio.py`'s own engine code (also already hardened). Pivoted to `anthropic_provider.py`'s extended-thinking round-trip, one layer deeper than the plain `thinking`-block case that already has its own dedicated end-to-end test (a real two-turn conversation proving the signed round trip survives intact).

Anthropic's real SDK also defines `RedactedThinkingBlock` (`type="redacted_thinking"`, `data: str`) -- returned whenever the model's own safety classifier flags part of its reasoning, a normal, documented, non-adversarial occurrence with extended thinking on (this adapter's own default for every Anthropic call), not a rare edge case. `generate()`'s response-parsing loop had no branch for it at all -- it matched none of the `if`/`elif`s and was silently dropped, never surfaced as a block, never yielded.

**Confirmed live** with the exact same fake-SDK-client pattern the existing signed-thinking round-trip test already uses, just swapping in a `redacted_thinking` block ahead of a `tool_use` block (the ordinary "reason, then call a tool" shape): the resulting `Message`, threaded unmodified into the next turn's history the same way `AgentLoop` always does, started with `tool_use` alone. Anthropic's extended-thinking + tool-use contract requires the leading thinking/redacted-thinking block be replayed verbatim on the follow-up request -- the real API would reject that turn outright, not just silently lose the reasoning content.

**Fixed** by representing it as a `ThinkingBlock` too, reusing the exact same `provider_data`-as-opaque-round-trip-storage mechanism the signed case already established rather than inventing a new content-block type: empty visible text (redaction's whole point -- there is none), the encrypted blob stored under a distinct `provider_data` key (`redacted_data`) from the signed case's `signature`, so the outgoing translation can tell the two apart and reconstruct the correct wire shape for each.

**Verified live** the redacted block now survives the identical two-turn round trip the signed case's own test already proves -- the second request's assistant turn correctly leads with `{"type": "redacted_thinking", "data": "ENCRYPTED_BLOB_XYZ"}` followed by the `tool_use` block. **Verified by reverting** and watching the new test fail exactly where the block should have been: `content[0]` was `tool_use`, not `redacted_thinking` -- the block simply wasn't there, the literal old bug reproducing itself. 1 new test, 709 -> 710 Python tests, all passing, `ruff check`/`format --check` clean. `docs/providers.md` gained a new subsection directly under the tool-call-id-collision fix, another recent fresh-eyes sweep of this same trio of adapters.

**Fifteen consecutive rounds now (46-60)** have found real bugs. Also closed a loose end from round 59: confirmed live that `image.py`'s synchronous Pillow decode is genuinely negligible (9ms for a realistic 4000x3000 12MP photo), closing the "sync call in async def" lens fully across every degrader with an actual number instead of a "probably fine."

**Next:** the completeness-audit backlog remains at three items needing external-dependency/scope decisions from the author (a code-execution sandbox tool, web search, image generation). The three infra-blocked items remain deferred (Tauri `csp: null`, RL harness sandboxing, inference batching); the quantization/`Budget` NaN-validation and explicit-partial-`Budget` gaps remain real-but-unreachable. Checked whether OpenAI's own adapter has a similar gap before closing this round out: it doesn't translate reasoning content at all, a deliberate, already-documented scope decision (see this adapter's own docstring on why `reasoning_effort` is left unmapped), not an unnoticed hole -- nothing to fix there.


## Two concurrent tool results silently fused into one misleading value in the Ollama adapter -- found on the request-building side, not response parsing, for the first time this streak

Round 61. Spent time first on `openai_provider.py`'s streaming tool-call accumulation and multi-turn translation -- already extremely well-hardened (index-keyed accumulation, empty-fragment handling, malformed-JSON degradation, `APIResponseValidationError` sibling coverage, image-in-tool-result rejection all explicitly tested). Pivoted to `ollama_provider.py`'s own request-building side (`_to_ollama_message`) -- every recent adapter fix in this streak (Gemini tool-call ids, Anthropic redacted thinking) targeted RESPONSE parsing; this is the first to target how an adapter builds its OUTGOING request instead.

`_to_ollama_message` accumulates a `TextBlock`'s text and, separately, each `ToolResultBlock`'s text into one flat `text_parts` list as it walks a message's content, then joins everything at the end with `"".join(text_parts)` -- no separator at all. Harmless for the overwhelmingly common single-block message, which is why this had gone unnoticed through multiple prior Ollama-adapter fix rounds (image support, malformed-NDJSON handling, tool-id collision checks) -- none of them happened to construct a `Message` with more than one `ToolResultBlock`. But `AgentLoop` builds exactly that shape every time a model turn issues more than one tool call: `Message(role="user", content=list(results))`, one `ToolResultBlock` per concurrently-dispatched call (`asyncio.gather` over the whole round) -- ordinary use, not a contrived edge case.

**Confirmed live:** two tool results, `"4"` and `"2"`, glued into the single wire-format string `"42"` -- not just losing which result came from which tool (an honest, already-documented limitation of this backend having no dedicated tool-result role at all), but actively fusing two distinct values into a DIFFERENT, misleading one the model on the other end has no way to separate back out. Confirmed end-to-end through the real `OllamaProvider.generate()` against a faked `httpx` transport too, not just the translation function in isolation: a realistic two-tool-call turn (`calculate` -> `"4"`, `file_size` -> `"2"`) produced the exact wire payload `{"role": "user", "content": "42"}` sent to `/api/chat`.

**Fixed** with a newline separator (`"\n".join(text_parts)`) instead of the empty-string join -- a strict improvement with zero regression for the common single-block case, since joining a one-element list is identical either way. **Verified live** the two results above now translate to `"4\n2"`, not `"42"`. **Verified by reverting** and watching the new test fail with the literal old bug's own fused value reproducing itself (`'42' == '4\n2'` -- false). 1 new test, 710 -> 711 Python tests, all passing, `ruff check`/`format --check` clean. `docs/providers.md` gained a new subsection directly under the redacted_thinking fix, and the existing "no dedicated tool-result role" bullet was updated to distinguish the accepted attribution loss from this real, separate value-corruption bug.
...
## `routing.yaml`'s own "zero config, always works" promise had one broken chain -- `audio` could never resolve, even with `mock` available

Round 63. Checked `core/sarva/eval/benchmarks.py` (still reports honest 0.0 accuracy, the grader fix holds, no regression) and `core/sarva/agent/events.py`'s `LEGAL` state-transition table (every real transition the loop performs is a legal entry; `AgentState.INTERRUPTED` is declared reachable but never actually constructed anywhere -- a dead terminal state, not a live bug). Skimmed `foundry/sarva_foundry/model/attention.py`/`layers.py`/`transformer.py` and `inference/generate.py`/`kv_cache.py` -- all extremely heavily hardened already, including `test_kv_cache.py`'s own existing proof that cached and full-recompute decoding produce token-identical output. Swept `core/sarva/providers/data/models.yaml`/`routing.yaml` themselves instead -- the actual shipped config data, not the well-tested `Router` code that reads it.

`routing.yaml`'s own header comment makes an explicit promise: "mock is always last so the CLI and test suite work with zero config." Its `audio: [mock]` chain -- the ONLY entry in that chain -- violated that promise: `mock`'s own `models.yaml` entry declared `modalities_in: [text, image, document]`, no `audio`. Confirmed live against the real shipped YAML files: `Router.pick(TaskClass.AUDIO, needs={Modality.AUDIO})` raised `LookupError` even with `mock` marked available -- the one case "always available" is supposed to guarantee never happens. `MockProvider.generate()` doesn't actually inspect modality at all (it just echoes the last user message's text), so this wasn't a real technical limitation being honestly reflected in the config -- it was a plain data omission. `tests/conformance/test_provider.py` already has a passing `TaskClass.VISION` test proving the identical pattern works for vision; there was conspicuously no `TaskClass.AUDIO` equivalent, which would have caught this immediately.

**Traced reachability carefully before deciding whether to fix it:** `AgentLoop` only ever constructs with `task_class=MAIN` or `SUBTASK` across every real CLI/server/subagent code path -- `TaskClass.AUDIO` (and `ESCALATION`) are defined in the enum but never actually invoked anywhere live. A real audio attachment routes through `AgentLoop`'s own `LookupError`-triggered degrader fallback instead, which correctly falls back to the `MAIN` chain with `needs={TEXT}` and already works fine. So this bug is currently latent in every shipped flow, not something an ordinary `sarva run`/`sarva chat` user hits today.

**Fixed anyway, not deferred as unreachable the way the quantization/`Budget` gaps are documented:** unlike those (which need a real design decision about new validation/clamping semantics for a genuinely ambiguous case), this fix is a one-line, zero-risk data correction restoring a promise the config file makes about itself, with MockProvider's own indifference to modality confirming there's no real behavior change for anything that currently works. Added `audio` (and `video`, for the identical reason, ahead of any future `TaskClass.VIDEO`) to `mock`'s declared `modalities_in`.

**Verified live** the fix resolves the identical `Router.pick(TaskClass.AUDIO, ...)` call that previously raised. **Verified by reverting** and watching the new test fail with the literal old `LookupError` reproducing itself. 1 new test, 713 -> 714 Python tests, all passing, `ruff check`/`format --check` clean (plus a manual check that no test anywhere asserted `mock`'s exact old `modalities_in` set, which would have needed updating). `docs/providers.md` gained a new subsection directly under the model-registry chapter's own `UnknownModelError`/CLI `--model` fix.
